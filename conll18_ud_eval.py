...
def load_conllu(file):
    ...
    while True:
        ...
        if sentence_start is None:
            ...
            ud.sentence_spans.append(UDSpan(sentence_start, 0))
        if not line:
            ...
            ud.sentence_spans[-1].end = len(ud.words) - 1
            sentence_start = None
            continue
...
def evaluate(gold_ud, system_ud):
    ...
    return {
        ...
        "MLAS": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL], w.columns[UPOS], w.columns[FEATS],
                                ...
                                filter_fn=lambda w: w.is_content_deprel),        "BLEX": alignment_score(alignment, lambda w, ga: (ga(w.parent), w.columns[DEPREL],
